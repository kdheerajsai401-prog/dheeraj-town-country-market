...
// Featured = sale items that also have a photo
const FEATURED = SAMPLE_PRODUCTS.filter((p) => p.salePrice && p.image).slice(0, 8)

// Category-colour fallback backgrounds when no image is available
          ...
          {FEATURED.map((product) => {
            ...
            return (
              <Link
                ...
                className="group flex flex-col bg-white rounded-card shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-warm-surface"
              >
                {/* Product image */}
                <div
